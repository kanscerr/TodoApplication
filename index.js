...
app.post('/login', (req, res) => {
    if(req.body.actionInfo && req.body.formData){
        model.findOne(
            ...
            (error, result) => {
                if(result){
                    global.sessionId = result.userId; //to use as sessionData for other CRUD operations
...
app.post('/addTodo', (req, res) => {
    if(req.body.actionInfo && req.body.sessionData.userId==userId && req.body.formData){
        const newData = {taskNumber: req.body.formData[0].taskNumber, todo : req.body.formData[0].todo, isDeleted : false}
    ...
    else if(req.body.sessionData.userId!=userId){
...
app.post('/getTodo', (req, res) => {
    let todoArr = []
    if(req.body.actionInfo && req.body.sessionData.userId==sessionId){
        model.findOne(
            {userId : req.body.sessionData.userId},
            (error, result) => {
                if(result){
    ...
    else if(req.body.sessionData.userId!=sessionId){
        res.send({error : "User not valid"});
    }
    else{
        res.send({error : 'Not enough information'})
...
app.post('/getOne', (req, res) => {
    if(req.body.actionInfo && req.body.sessionData.userId==sessionId && req.body.formData){
        model.findOne(
